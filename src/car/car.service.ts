...
export class CarService {
  ...
  async findAll(where: CarFilter, sortBy: SortbyDto): Promise<Car[]> {
    return this.carRepository.find({
      relations: ['owner'],
      where,
      order:
        sortBy != undefined ? { [sortBy.sortby]: sortBy.orderby } : undefined,
    });
  }
  ...
  async findAllAvailable(
    ...
    return this.carRepository.find({
      join: {
        alias: 'cars',
        innerJoinAndSelect: { availability: 'cars.availability' },
      },
      order:
        sortBy != undefined ? { [sortBy.sortby]: sortBy.orderby } : undefined,
      where: (qb: SelectQueryBuilder<Car>) => {
        ...
        if (whichAvailableOn != undefined) {
          ...
          if (whichAvailableOn.duration != undefined) {
            qb.andWhere('availability.start_date > :time1::timestamp', {
              time1: whichAvailableOn.duration[0].toISOString(),
            }).andWhere('availability.end_date < :time2::timestamp', {
